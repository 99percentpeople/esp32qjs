...
def replace(text: str, old: str, new: str, count: int = 1) -> str:
    if text.count(old) != count:
        raise ValueError('Unexpected EAP source anchor: ' + old)
    return text.replace(old, new)


def function(text: str, name: str) -> str:
    match = re.search(r'^(?:static )?(?:void|int|esp_err_t) ' + re.escape(name) + r'\([^;{}]*\)\n\{', text, re.M)
    if not match:
        raise ValueError('Missing EAP function: ' + name)
