...
def replace(text: str, before: str, after: str, count: int = 1) -> str:
    if text.count(before) != count:
        raise ValueError('Unexpected reviewed NAN source fragment: ' + before[:90])
    return text.replace(before, after)


def function(text: str, name: str, transform) -> str:
    # Match a definition, never an earlier call or an API name in a comment.
    matches = list(re.finditer(r'^(?:static )?[\w *]+\b' + re.escape(name) + r'\([^;{}]*\)\n\{', text, re.M))
    if len(matches) != 1:
        raise ValueError('Expected one NAN definition: ' + name)
