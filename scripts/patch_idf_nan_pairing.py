...
def replace(source, before, after, count=1):
    if source.count(before) != count:
        raise ValueError('Unexpected pairing fragment: ' + before[:100])
    return source.replace(before, after)


def function(source, name, transform):
    matches = list(re.finditer(r'^[A-Za-z_][\w *]+\b' + re.escape(name) + r'\([^;{}]*\)\n\{', source, re.M))
    if len(matches) != 1:
        raise ValueError('Expected one pairing function: ' + name)
