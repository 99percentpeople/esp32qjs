...
import struct
import tempfile
from patch_idf_smartconfig_stack import patch_archive as patch_stack_archive
...
def replace(text: str, before: str, after: str, count: int = 1) -> str:
    if text.count(before) != count:
        raise ValueError('Unexpected SmartConfig source anchor: ' + before)
    return text.replace(before, after)


def function(text: str, name: str) -> str:
    import re
    match = re.search(r'^(?:static )?(?:void|int|esp_err_t) \*?' + name + r'\([^;{}]*\)\n\{', text, re.M)
    if not match:
        raise ValueError('Missing SmartConfig function: ' + name)
