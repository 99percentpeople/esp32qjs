...
class Object:
    def __init__(self, source: bytes):
        if source[:7] != b"\x7fELF\x01\x01\x01" or struct.unpack_from("<H", source, 16)[0] != 1:
        ...
        if width != 40 or not count or names >= count or self.shoff + width * count > len(source):
            raise ValueError("Invalid AP object section table")
        ...
        if len(tables) != 1:
            raise ValueError("Expected one AP object symbol table")
        self.table_index = tables[0]
        table = self.sections[self.table_index]
        if table[9] != 16 or table[5] % 16:
            raise ValueError("Invalid AP object symbols")
    ...
    def body(self, index: int) -> bytes:
        ...
        if offset + size > len(self.source):
            raise ValueError("AP object section exceeds input")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="ap-prestart-", dir=args.output.parent) as directory:
        ...
        result = Object(linked.read_bytes())
