...
## Testing
The default validation target is `python scripts/remote.py test`. It runs host C tests plus the JS modules enabled by the active board's runtime feature set, and it reflashes the latest firmware before board-backed JS tests so C-side changes are not left stale on the device. Use `--scope c`, `--scope js`, `--module ...`, and `--network` to narrow or extend coverage when needed. Use `--no-flash-firmware` and `--no-flash-fs` only when you intentionally want to reuse what is already on the board.
