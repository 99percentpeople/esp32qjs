...
import build_tools.cli as REMOTE_CLI
import build_tools.profiles as REMOTE_PROFILES
import build_tools.build as REMOTE_BUILD
import build_tools.device_tests as REMOTE_DEVICE_TESTS
import build_tools.flash as REMOTE_FLASH
import build_tools.server as REMOTE_SERVER


class RemoteConfigTests(unittest.TestCase):
    def config(self):
        args, mcu, context = REMOTE_CLI.parse_args(["show-config"])
        return REMOTE_PROFILES.build_project_config(args, mcu, context)
    ...
    def test_remote_entrypoint_delegates_to_stable_responsibility_modules(self):
        ...
        module_dir = ROOT / "scripts" / "build_tools"
        expected_modules = (
            "profiles",
            "build",
            "flash",
        ...
        self.assertIn("from build_tools.cli import main", entrypoint)
        self.assertNotIn("import *", entrypoint)
        self.assertIs(REMOTE.main, REMOTE_CLI.main)
    ...
    def test_default_context_is_fixed_layout_and_stays_under_build_root(self):
        ...
        self.assertEqual(
            REMOTE_PROFILES.resolve_build_path("scratch/profile"),
    ...
    def test_external_build_directory_requires_explicit_opt_in(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            config = REMOTE_PROFILES.build_project_config(args, mcu, context)
    ...
    def test_build_cleanup_is_limited_to_build_root_without_opt_in(self):
        with tempfile.TemporaryDirectory(dir=ROOT) as temp_name:
            ...
            with self.assertRaisesRegex(SystemExit, "unsafe build directory"):
                REMOTE_BUILD.safe_remove_build_dir(path, allow_external=False)
            self.assertTrue(path.exists())
        for path in (Path(path.anchor), Path.home(), ROOT, ROOT / "build"):
            with self.subTest(path=path), self.assertRaisesRegex(
                SystemExit, "unsafe build directory"
            ):
                REMOTE_BUILD.safe_remove_build_dir(path, allow_external=True)
    ...
    def test_idf_actions_install_the_project_local_esptool_config(self):
        source = (ROOT / "scripts" / "build_tools" / "build.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_serial_tools_scope_esptool_configuration_to_child_processes(self):
        build_source = (
            ROOT / "scripts" / "build_tools" / "build.py"
        ).read_text(encoding="utf-8")
        flash_source = (
            ROOT / "scripts" / "build_tools" / "flash.py"
        ).read_text(encoding="utf-8")
        server_source = (
            ROOT / "scripts" / "build_tools" / "server.py"
        ).read_text(encoding="utf-8")
    ...
    def test_rfc2217_stop_is_pid_scoped(self):
        source = (ROOT / "scripts" / "build_tools" / "server.py").read_text(
            encoding="utf-8"
        )
    ...
    def test_explicit_context_drives_mcu_and_rejects_incomplete_input(self):
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            with self.assertRaisesRegex(SystemExit, "Build Context is incomplete"):
                REMOTE_PROFILES.load_build_context(str(context))

        selected = REMOTE_PROFILES.load_build_context(
    ...
    def test_check_js_includes_the_context_flash_tree(self):
        ...
        with patch.object(
            ...
            REMOTE_DEVICE_TESTS.run_js_syntax_check(selected)
    ...
    def test_wireless_hardware_e2e_is_opt_in_and_separate(self):
        args, _, _ = REMOTE_CLI.parse_args(
            ["test", "--scope", "js", "--module", "espnow",
             "--wireless-hardware"]
        )
        module = REMOTE_DEVICE_TESTS.resolve_js_modules(["espnow"])[0]
        hardware_cases = [
        ...
        self.assertEqual(
            REMOTE_DEVICE_TESTS.resolve_js_test_capabilities(args),
            {"wireless-hardware"},
        )
        self.assertEqual(len(hardware_cases), 1)
        self.assertTrue(hardware_cases[0].path.endswith("-hardware.js"))
        self.assertEqual(
            REMOTE_DEVICE_TESTS.JS_TEST_CAPABILITY_FLAGS["wireless-hardware"],
            "--wireless-hardware",
        )
    ...
    def test_csi_hardware_matrix_is_opt_in_and_separate(self):
        ...
        module = REMOTE_DEVICE_TESTS.resolve_js_modules(["wifi_csi"])[0]
        hardware_cases = [
            case for case in module.cases
        ...
        self.assertEqual(
            REMOTE_DEVICE_TESTS.JS_TEST_CAPABILITY_FLAGS["csi-hardware"],
            "--csi-hardware",
        )
    ...
    def test_csi_long_soak_requires_a_second_explicit_capability(self):
        ...
        self.assertEqual(
            REMOTE_DEVICE_TESTS.JS_TEST_CAPABILITY_FLAGS["csi-soak"],
            "--csi-soak",
        )
    ...
    def test_camera_csi_coexistence_requires_both_hardware_capabilities(self):
        ...
        module = REMOTE_DEVICE_TESTS.resolve_js_modules(["wifi_csi_camera"])[0]
    ...
    def test_csi_coexistence_modules_require_their_real_dependencies(self):
        ...
        for module_name, (features, capabilities) in expected.items():
            with self.subTest(module=module_name):
                module = REMOTE_DEVICE_TESTS.resolve_js_modules([module_name])[0]
    ...
    def test_espnow_modules_use_the_public_runtime_feature_key(self):
        ...
        for module_name in ("espnow", "wifi_csi_espnow"):
            with self.subTest(module=module_name):
                ...
                enabled, note = REMOTE_DEVICE_TESTS.resolve_js_modules_for_runtime(
    ...
    def test_csi_hardware_pass_keeps_bounded_structured_evidence(self):
        case = REMOTE_DEVICE_TESTS.JsTestCase(
        ...
        with (
            ...
            result = REMOTE_DEVICE_TESTS.run_js_test_case(session, case)
        ...
        bounded = REMOTE_DEVICE_TESTS.format_js_case_details({"payload": "x" * 5000})
        self.assertLessEqual(
            len(bounded.encode("utf-8")),
            REMOTE_DEVICE_TESTS.JS_TEST_DETAILS_MAX_BYTES,
        )
        self.assertEqual(
            json.loads(bounded),
    ...
    def test_js_case_command_retries_once_when_start_marker_is_missing(self):
        case = REMOTE_DEVICE_TESTS.JsTestCase("modules/core/eval.js", timeout_seconds=5.0)
        session = SimpleNamespace(process=SimpleNamespace(poll=lambda: None))
        ...
        self.assertIn(REMOTE_DEVICE_TESTS.JS_TEST_CASE_START_PREFIX, first_command)
    ...
    def test_js_test_build_creates_a_real_context_and_selected_tree(self):
        config = self.config()
        modules = REMOTE_DEVICE_TESTS.resolve_js_modules(["bitmap", "camera-bitmap"])
        test_config = REMOTE_DEVICE_TESTS.js_test_build_config(
            config,
            modules=modules,
    ...
    def test_js_test_build_disables_debug_gc_only_for_psram_contexts(self):
        ...
        with tempfile.TemporaryDirectory() as temp_name:
            ...
            test_config = REMOTE_DEVICE_TESTS.js_test_build_config(config)
    ...
    def test_js_tests_flash_and_monitor_the_instrumented_context(self):
        ...
        with (
            ...
            REMOTE_DEVICE_TESTS.run_js_tests(config, (), False, set(), True, False)
    ...
    def test_monitor_no_reset_and_workspace_flash_semantics_remain_explicit(self):
        config = self.config()
        config = dataclasses.replace(config, target="/dev/ttyACM0")
        with (
            ...
            self.assertIn(
                "monitor --no-reset",
                " ".join(REMOTE_SERVER.monitor_cmd(config, no_reset=True)),
            )
        with (
            ...
            REMOTE_FLASH.flash(config, build_first=False)
            self.assertEqual(resolve.call_args.kwargs["exclude_entries"], ("workspace",))
            REMOTE_FLASH.flash(config, build_first=False, initialize_workspace=True)
    ...
    def test_flash_subprocess_receives_only_the_project_tool_environment(self):
        config = dataclasses.replace(self.config(), target="/dev/ttyACM0")
    ...
    def test_future_case_and_serial_helpers_keep_their_contracts(self):
        case = REMOTE_DEVICE_TESTS.JS_TEST_MODULE_MAP["future"].cases[0]
        self.assertTrue(case.reset_before)
        self.assertTrue(case.reset_after)
        self.assertIsNone(REMOTE_SERVER.find_last_output_line_with_prefix("__MARK__:r", "__MARK__:", complete_only=True))
        self.assertEqual(
            REMOTE_SERVER.find_last_output_line_with_prefix("__MARK__:ready\r\njs> ", "__MARK__:", complete_only=True),
            "__MARK__:ready",
        )
        self.assertIsNone(REMOTE_PROFILES.merged_optional_int({"PORT": "  "}, {}, "PORT"))
        self.assertEqual(REMOTE_PROFILES.merged_optional_int({"PORT": " 42 "}, {}, "PORT"), 42)
