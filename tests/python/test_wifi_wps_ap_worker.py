"""Deferred production AP worker scheduling/ownership tests; AST only during implementation.
...
class WPSAPWorker(unittest.TestCase):
    def test_production_worker_retains_ipc_and_orders_retirement(self):
        ...
        source = declarations((ROOT / 'components/esp32_mquickjs/src/modules/wifi_wps/esp32_mquickjs_wifi_wps_ap_worker.c').read_text())
        ...
        code += declarations((internal / 'esp32_mquickjs_wifi_wps_ap_result.h').read_text())
        code += declarations((internal / 'esp32_mquickjs_wifi_wps_ap_sdk.h').read_text())
        code += declarations((internal / 'esp32_mquickjs_wifi_wps_ap_worker.h').read_text())
        station = (ROOT / 'components/esp32_mquickjs/src/modules/wifi_wps/esp32_mquickjs_wifi_wps_worker.c').read_text()
        begin = station.index('bool esp32_mquickjs_wifi_wps_config_valid(')
        validation = station[begin:station.index('\n}\n', begin) + 3]
        compile_run(self, code + BOUNDARIES + validation + source + IMPLEMENTATIONS + MAIN)
...
int main(void) {
    ...
    esp32_mquickjs_wifi_wps_ap_worker_t *w=NULL;
    memcpy(config.pin,"12345671",9);
    assert(esp32_mquickjs_wifi_wps_ap_worker_create(&config,&w)==ESP_ERR_INVALID_ARG && !allocations);
    ...
    assert(esp32_mquickjs_wifi_wps_ap_worker_create(&config,&w)==ESP_OK);
    assert(esp32_mquickjs_wifi_wps_ap_worker_start(w)==ESP_ERR_INVALID_STATE && !start_calls);
    assert(esp32_mquickjs_wifi_wps_ap_worker_prepare(w)==ESP_OK && !start_calls);
    assert(esp32_mquickjs_wifi_wps_ap_worker_prepare(w)==ESP_ERR_INVALID_STATE);
    assert(esp32_mquickjs_wifi_wps_ap_worker_start(w)==ESP_OK && start_calls==1);
    assert(esp32_mquickjs_wifi_wps_ap_worker_release(&w)==ESP_ERR_INVALID_STATE && allocations==1);
    for(size_t i=0;i<sizeof(w->config);i++)assert(!((uint8_t *)&w->config)[i]);
    uint8_t pin[8];
    assert(esp32_mquickjs_wifi_wps_ap_worker_pin(w,pin,false)==ESP_OK && !memcmp(pin,"12345670",8));
    assert(!commit_calls);
    for(size_t i=0;i<sizeof(w->pin);i++)assert(!((uint8_t *)&w->pin)[i]);
    assert(esp32_mquickjs_wifi_wps_ap_worker_pin(w,NULL,true)==ESP_OK && commit_calls==1);
    assert(esp32_mquickjs_wifi_wps_ap_worker_finish_capture(w)==ESP_ERR_INVALID_STATE);
    native.terminal=true;
    create_error=301;
    assert(esp32_mquickjs_wifi_wps_ap_worker_finish_capture(w)==301 && !retire_calls);
    create_error=0;start_error=302;
    unsigned completed_stops=stop_calls;
    assert(esp32_mquickjs_wifi_wps_ap_worker_finish_capture(w)==302 && stop_calls==completed_stops);
    start_error=0;
    assert(esp32_mquickjs_wifi_wps_ap_worker_finish_capture(w)==ESP_ERR_NOT_FINISHED && !retire_calls);
    fire_timer();stop_error=303;
    assert(esp32_mquickjs_wifi_wps_ap_worker_finish_capture(w)==303 && !retire_calls);
    stop_error=0;delete_error=304;
    assert(esp32_mquickjs_wifi_wps_ap_worker_finish_capture(w)==304 && !retire_calls);
    unsigned stopped=stops;
    delete_error=0;
    assert(esp32_mquickjs_wifi_wps_ap_worker_finish_capture(w)==ESP_OK && retire_calls==1 && stops==stopped);
    release_error=ESP_ERR_NOT_FINISHED;
    assert(esp32_mquickjs_wifi_wps_ap_worker_close(w)==ESP_ERR_NOT_FINISHED && !w->capture_retired);
    release_error=0;
    assert(esp32_mquickjs_wifi_wps_ap_worker_close(w)==ESP_ERR_NOT_FINISHED && timer_exists);
    fire_timer();
    assert(esp32_mquickjs_wifi_wps_ap_worker_close(w)==ESP_OK && w->retired && !w->identity);
    assert(esp32_mquickjs_wifi_wps_ap_worker_close(w)==ESP_OK);
    assert(esp32_mquickjs_wifi_wps_ap_worker_release(&w)==ESP_OK && !w && !allocations);
    reset_boundaries();

    /* Definite pre-handoff OOM permits cleanup of an owner with no native ID. */
    assert(esp32_mquickjs_wifi_wps_ap_worker_create(&config,&w)==ESP_OK);ipc_mode=1;
    assert(esp32_mquickjs_wifi_wps_ap_worker_prepare(w)==ESP_ERR_NO_MEM && !w->handoff_unknown);
    assert(esp32_mquickjs_wifi_wps_ap_worker_close(w)==ESP_OK && !creates);
    assert(esp32_mquickjs_wifi_wps_ap_worker_release(&w)==ESP_OK);
    reset_boundaries();

    /* An unconfirmed copy must retain its output and surface the raw IPC
     * error even when start itself succeeded. */
    assert(esp32_mquickjs_wifi_wps_ap_worker_create(&config,&w)==ESP_OK);
    assert(esp32_mquickjs_wifi_wps_ap_worker_prepare(w)==ESP_OK);
    assert(esp32_mquickjs_wifi_wps_ap_worker_start(w)==ESP_OK);
    ipc_mode=2;
    assert(esp32_mquickjs_wifi_wps_ap_worker_pin(w,pin,false)==909 && w->handoff_unknown);
    esp32_mquickjs_wifi_wps_ap_worker_status_t copy_status;
    assert(esp32_mquickjs_wifi_wps_ap_worker_status(w,&copy_status)==ESP_OK && copy_status.error==909);
    assert(pending.fn(pending.arg)==ESP_OK && !memcmp(w->pin,"12345670",8));
    assert(esp32_mquickjs_wifi_wps_ap_worker_release(&w)==ESP_ERR_INVALID_STATE);
    ...
    assert(esp32_mquickjs_wifi_wps_ap_worker_create(&config,&w)==ESP_OK);ipc_mode=2;
    assert(esp32_mquickjs_wifi_wps_ap_worker_prepare(w)==909 && w->handoff_unknown);
    esp32_mquickjs_wifi_wps_ap_worker_status_t status;
    w->native.identity=UINT32_MAX;
    assert(esp32_mquickjs_wifi_wps_ap_worker_status(w,&status)==ESP_OK && !status.native.identity && status.handoff_unknown);
    assert(esp32_mquickjs_wifi_wps_ap_worker_close(w)==ESP_ERR_INVALID_STATE);
    assert(esp32_mquickjs_wifi_wps_ap_worker_release(&w)==ESP_ERR_INVALID_STATE && allocations==1);
