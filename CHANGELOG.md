...
- Add `minimal` and `demo` application profiles with `--app` tooling,
  application sdkconfig overlays, generated Flash-sized partitions, and resource overlays.
- Replace development-board presets with ESP32-C3/ESP32-S3 MCU profiles plus generated
  Flash, PSRAM, and optional wiring overlays; unset pin defaults stay explicit.
...
- Add an optional bounded NVS string module with purge updates and explicit
  ...
  credential material.
- Expose the stable factory eFuse Base MAC as `sys.info().hardwareId` in
  `hw-xxxxxxxxxxxx` form.
