...
用户最新安排：长时间生命周期、内存/吞吐和共存验收在全部无线功能完成后集中执行；短竞争、错误注入和单功能正确性测试仍随实现执行。见[集中验收账本](investigations/2026-09-07-wifi-refactor-start.md)。第一阶段新增 timeout 取消失败保留原生存储的修复已通过 Host/构建，设备验证 not-run；本阶段新功能尚未开始。

**启动前提（不是当前结果声明）**：01 的 F-CORE 必须已通过，并确认当前分支相对实施基线的变化。新功能不要求 Wi-Fi 全部高级能力先完成，但共用的 native 生命周期、预算和生成物修改必须协调，最终完成混合负载验收。
