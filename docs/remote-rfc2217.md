...
If a profile defines a `workspace` image, normal `flash` and `flash-monitor`
exclude it. Initialize a new partition layout once with `flash
--erase-workspace`, or deliberately clear only that partition with
`flash-workspace`. `flash-fs` always updates only the system `storage` image.
...
TEST_JS_CONFIG='{"websocketUrl":"ws://192.168.1.10:18765"}' python scripts/remote.py test --scope js --module websocket --network
